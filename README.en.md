...
**Done:** the whole infrastructure works end-to-end on a real Linux guest —
boot, custom Alpine+Node.js image, host↔guest files, real `npm install`,
reaching a guest port from host JS, and ~198x faster warm restarts via
snapshots. A real, unmodified **Express** app installs, starts and answers
HTTP requests through this pipeline.

**Main problem:** v86 emulates **32-bit x86 only** (permanent, not a config
choice). Anything with a mandatory native dependency that has no `ia32`
build cannot run. This is what blocks **Vite** (Rollup ships no 32-bit
binary; its WASM fallback didn't take effect in the guest) and likely
**Next.js** (SWC is native too). Pure-JS frameworks work; native-dependent
toolchains don't. Second, smaller problem: real internet access needs a
self-hosted relay server (`wsproxy`), so this is not a "pure client" runtime.

See [STATUS.md](STATUS.md) for the condensed what-works / what-doesn't table.

## What's done

- ✅ Headless Linux boot (Phase 0)
- ✅ Custom Alpine + Node.js/npm guest image via Docker (Phase 1)
- ✅ 9p host↔guest file exchange (Phase 2)
- ✅ Real `npm install` from inside the guest — via relay (Phase 3)
- ✅ Host JS reaches a port the guest listens on, via a snapshot handoff
  between backends (Phase 4)
- ✅ Snapshots: cold boot 43s → warm restore 220ms (Phase 5)
- ✅ Express end-to-end: 68 packages installed, real `HTTP/1.1 200 OK`

## What's not done / the blockers

- ❌ **Vite dev server** — Rollup has no `ia32` build (see the main problem above)
- ⚠️ Next.js, native npm modules (`node-gyp`) — not attempted; same 32-bit risk class
- ⚠️ Real browser E2E (iframe, HMR over WebSocket, Playwright) — written, never run (no browser in the dev environment)
- ⚠️ Production networking needs a self-hosted relay

## Status: Phases 0-5 verified with real spikes; Phase 6 blocked; Phase 7 (this file)
