...
| [`Sandpack/nodebox-runtime`](https://github.com/Sandpack/nodebox-runtime) | Кастомная, **не open source** — только некоммерческое/личное использование либо "внутренние бизнес-цели" (формулировка нечёткая, трактовать консервативно) | **Только референс архитектуры**, не копировать код напрямую без отдельной юридической проверки лицензии. Идеи (как устроен Next.js SSR-режим, как выключается Service Worker для SSR) — использовать как вдохновение и переписывать с нуля. |
...
8. Лицензионный риск при заимствовании кода из Nodebox (см. таблицу выше) — при любом сомнении переписывать с нуля, а не копировать.
